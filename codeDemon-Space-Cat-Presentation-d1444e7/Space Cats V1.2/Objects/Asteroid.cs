    ...
    class Asteroid : IEnemyShip, IExplodable
    {
        ...
        public int Size
        {
            ...
            set
            {
                ...
                Width = SpriteWidth / (MAXX_SIZE - z_size)/6;
                Height = SpriteHeight / (MAXX_SIZE - z_size)/6;
        ...
        public static void Initialize(ContentManager content, AsteroidDensity density)
        {
            ...
            for (int i = 1; i <= 4; i++)
            {
                zs_images.Add(content.Load<Texture2D>("Content\\Images\\Asteroids\\MedDirtRock" + i));
                zs_images.Add(content.Load<Texture2D>("Content\\Images\\Asteroids\\SmallDirtRock" + i));
            }
